...
int Database::addUser(string username, string password)
{
	...
	_usersMapByName.insert({ username, _users.size() - 1 });
...
bool Database::addPrivateMessage(string sender, string target, string message)
{
	...
	_messages.push_back(Message(sender, _users[targetUser].getUserID(), message));
	return true;
}
...
vector<Message> Database::getPrivateMessage(int userID)
{
	...
	for (auto &m : _messages)
	{
		if (m.getDest() == userID) strings.push_back(m);
	}
	return strings;
}
